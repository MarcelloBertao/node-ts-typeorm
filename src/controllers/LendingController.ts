...
export class LendingController {
  ...
  async delete(req: Request, res: Response) {
    const { idlending } = req.params
    if (!idlending) return res.status(400).json({ "message": "Empréstimo necessário!" })

    try {
      await lendingRepository
        .createQueryBuilder('lendings')
        .softDelete()
        .where("id = :id", { id: idlending })
        .execute()

      return res.status(202).json({ "message": "Empréstimo deletado com sucesso!" })
    } catch (error) {
      console.log(error)
      return res.status(500).json({ "message": "Internal Server Error!" })
    }
  ...
  async restore(req: Request, res: Response) {
    ...
    try {
      ...
      return res.status(202).json({ "message": "Empréstimo restaurado com sucesso!" })
    } catch (error) {
      console.log(error)
      return res.status(500).json({ "message": "Internal Server Error!" })
    }
